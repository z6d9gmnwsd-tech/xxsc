...
export default function AuthModal({ isOpen, onClose, onSuccess }: AuthModalProps) {
  ...
  const resetForm = () => {
    ...
    setError(''); setSuccess(''); setAppealPhone(''); setAppealEvidence(''); setAppealCodeWord('')
  }
  ...
  const handleAppeal = async () => {
    ...
    setLoading(true); setError('')
    try {
      const { error } = await supabase.from('appeals').insert({ phone: appealPhone, evidence: sanitizeInput(appealEvidence), code_word: sanitizeInput(appealCodeWord) })
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 transition-all duration-300 ${isVisible ? 'opacity-100' : 'opacity-0'}`} onClick={handleClose}>
      <div className={`absolute inset-0 bg-black/40 transition-opacity duration-300 ${isVisible ? 'opacity-100' : 'opacity-0'}`} style={{ backdropFilter: 'blur(4px)' }} />
        ...
        <div className="p-6 max-h-[60vh] overflow-y-auto scroll-container">
          ...
          {step === 'appeal' && (
            <div className="space-y-4 animate-fade-in">
              ...
              <div><label className="block text-sm mb-2 font-medium" style={{ color: '#666' }}>您申诉的手机号码 *</label>
                <input type="tel" className="input" placeholder="请输入被占用的手机号" value={appealPhone} onChange={e => setAppealPhone(e.target.value.replace(/\D/g, '').slice(0, 11))} disabled={loading} maxLength={11} /></div>
              <div><label className="block text-sm mb-2 font-medium" style={{ color: '#666' }}>提供该手机号为本人的证据 *</label>
                <textarea className="input min-h-[80px] resize-none" placeholder="例如：手机营业厅App号码截图、缴费记录等，请详细描述" value={appealEvidence} onChange={e => setAppealEvidence(e.target.value)} disabled={loading} maxLength={500} /></div>
              <div><label className="block text-sm mb-2 font-medium" style={{ color: '#666' }}>设置暗号 *</label>
              ...
              <button onClick={handleAppeal} disabled={loading} className="btn-primary w-full disabled:opacity-50">{loading ? '提交中...' : '提交申诉'}</button>
              <div className="text-center">
